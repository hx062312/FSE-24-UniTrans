...
import signal
from process_valid_inputs import (
    TimeoutHandler,
    run_cmd,
    locate_params_py,
    refine_code_py,
    locate_function_name_java,
    refine_code_ja,
    has_array,
    locate_function_name_cpp,
    refine_code_cpp,
)
        ...
        for line in tqdm(fr.readlines()):
            ...
            if test_cases != [] and func_name == None:
                ...
                with jsonlines.open(out_file, "a") as fw:
                    fw.write(
                        {
                            "id": id,
                            "feedbacks": [("", "OK") for _ in range(test_case_num)],
                        }
                    )
                correct += 1
                continue
                ...
                with jsonlines.open(out_file, "a") as fw:
                    fw.write(
                        {
                            "id": id,
                            "feedbacks": [("", "") for _ in range(test_case_num)],
                        }
                    )
                continue
            ...
            for case_id, pair in enumerate(test_cases):
                if "Outputs (void):\n" not in pair:
                    ...
                    param_lst1 = [
                        i.split("=")[1] for i in param_lst if i != "" and "=" in i
                    ]
                    case = ",".join(param_lst)
                    case1 = param_lst1[0]
                    script = ""
                    with open(
                        "./cleaned_data/test_case_gen_scripts/unit_test.py",
                        encoding="utf-8",
                    ) as fr:
                        for line in fr.readlines():
                            if "# TO_FILL_FUNC" in line:
                                ...
                                if len(param_lst) <= 1:
                                    script += line.replace(
                                        "# ACT_OUT", f"act_out = {func_name}({case1})"
                                    )
                                else:
                                    param_lines = ""
                                    for param in param_lst:
                                        param_lines += (
                                            line.replace("# ACT_OUT", f"{param}") + "\n"
                                        )
                                    script += param_lines + line.replace(
                                        "# ACT_OUT", f"act_out = {func_name}({case})"
                                    )
                            elif "# EXP_OUT" in line:
                                if (
                                    outputs.strip() == "inf"
                                    or outputs.strip() == "-inf"
                                ):
                                    outputs = f"float('{outputs.strip()}')"
                                script += line.replace(
                                    "# EXP_OUT", f"exp_out = {outputs}"
                                )
                            else:
                                script += line
                else:
                    ...
                    with open(
                        ...
                        for line in fr.readlines():
                            if "# TO_FILL_FUNC" in line:
                                ...
                                script += line
                with open(
                    "./cleaned_data/test_case_gen_scripts/tmp_scripts/tmp.py", "w"
                ) as fw:
                    fw.write(script)
                # print(script)
                # assert False
        ...
        for line in tqdm(fr.readlines()):
            ...
            for pair in test_cases:
                if "Outputs (void):\n" not in pair:
                    ...
                    script = ""
                    with open(
                        "./cleaned_data/test_case_gen_scripts/unit_test.java",
                        encoding="utf-8",
                    ) as fr:
                        for line in fr.readlines():
                            if "// TO_FILL_FUNC" in line:  # done
                                ...
                                if len(param_lst) <= 1:  # done
                                    param_lst_one = [
                                        i for i in param_lst if i != "" and "=" in i
                                    ][0]
                                    head = param_lst_one.split("=")[0]
                                    var = head.strip().split()[-1]
                                    param_lines = (
                                        line.replace("// ACT_OUT", f"{param_lst_one}")
                                        + "\n"
                                    )
                                    script += param_lines + line.replace(
                                        "// ACT_OUT",
                                    ...
                                    script += param_lines + line.replace(
                                        "// ACT_OUT",
                                        f"{act_return_type} act_out = {func_name}({case});",
                                    )
                            elif "// EXP_OUT" in line:
                                script += line.replace(
                                    "// EXP_OUT",
                                    f"{exp_return_type} exp_out = {outputs};",
                                )  # done
                                ...
                                if has_array(exp_return_type):
                                    ...
                                    line1 = line.replace(
                                        "// COMPARE",
                                        f'if(act_out.equals(exp_out)) System.out.println("OK");\n',
                                    )
                                    line2 = line.replace(
                                        "// COMPARE",
                                    ...
                                    line1 = line.replace(
                                        "// COMPARE",
                                        f'if(act_out == exp_out) System.out.println("OK");\n',
                                    )
                                    line2 = line.replace(
                                        "// COMPARE",
                                        f'else {{\nSystem.out.print("Expected Output:{outputs}\\nActual Output:");\nSystem.out.print(act_out);\nSystem.out.print("\\nExpected output and actual output are not equal!");}}',
                        ...
                        for line in fr.readlines():
                            if "// TO_FILL_FUNC" in line:
                                ...
                                if len(param_lst) <= 1:
                                    ...
                                    script += param_lines + line.replace(
                                        "// ACT_OUT", f"{func_name}({case});"
                                    )
                            elif "// COMPARE" in line:
                                script += line.replace(
                                    "// COMPARE", f'System.out.println("OK");'
                                )
                            else:
                                script += line
                # print(script)
                # assert False
                with open(
                    "./cleaned_data/test_case_gen_scripts/tmp_scripts/tmp.java", "w"
                ) as fw:
                    fw.write(script)
                # print(script)
                # assert False
        ...
        for line in tqdm(fr.readlines()):
            ...
            for pair in test_cases:
                if "Outputs (void):\n" not in pair:
                    ...
                    script = ""
                    with open(
                        "./cleaned_data/test_case_gen_scripts/tmp_scripts/backtrace/example/backward_test.cc",
                        encoding="utf-8",
                    ) as fr:
                        for line in fr.readlines():
                            if "// TO_FILL_FUNC" in line:
                ...
                try:
                    result = run_cmd(
                        f"cd ./cleaned_data/test_case_gen_scripts/tmp_scripts/backtrace"
                        f"&& make && ./backtrace_benchmark"
                    )
                    signal.alarm(0)
                except Exception as ret:
                    cases_errs.append((pair, f"Timeout Error"))
                ...
                if "make done" not in result[0] and result[1] != "":
                    ...
                    "make done" in result[0]
                    and result[0].split("\n")[-1] != "OK"
                    and result[0].strip().split("\n")[-1]
                    == "Expected output and actual output are not equal!"
                ):
                    err_msg = result[0].replace("\n", ",")
                    cases_errs.append((pair, f"Logic Error:\n{err_msg}"))
...
if __name__ == "__main__":
    ...
    if dst_lang == "python":
        fetch_exe_ret_py(
            target_file, out_file, test_case_file, test_case_num, timeout=10
        )
    elif dst_lang == "java":
        fetch_exe_ret_ja(
            target_file, out_file, test_case_file, test_case_num, timeout=10
