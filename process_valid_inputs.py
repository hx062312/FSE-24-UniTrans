...
# python process_valid_inputs.py --model deepseek --src_lang java --dst_lang python
import json
import re
...
def post_process_py(model, keep_all, dst_lang, timeout=5, start=1):
    ...
    count = 0
    with open(
        f"./cleaned_data/{model}/valid_inputs/valid_inputs_python.jsonl",
        encoding="utf-8",
    ) as fr:
        for line in tqdm(fr.readlines()):
            ...
            for item_i in item:
                if model != "deepseek":
                    ...
                    if len(found) != 0:
                        ...
                        print("NO CASES! ", id)
                case_lst = [
                    i.strip() for i in re.split(r"Input_\d+:", item_i) if i != ""
                ]
                for case in case_lst:
                    ...
                    param_lst2 = [
                        pa for pa in param_lst if pa != "" and "=" in pa
                    ]  # var = value
                    ...
                    if param_lst2 != []:
                        ...
                        with open(
                            "./cleaned_data/testable_samples.jsonl", encoding="utf-8"
                        ) as fr:
                        ...
                        script = ""
                        with open(
                            "./cleaned_data/test_case_gen_scripts/gen_case.py",
                            encoding="utf-8",
                        ) as fr:
                            for line_script in fr.readlines():
                                if line_script.strip() == "# TO_FILL_FUNC":
                                    ...
                                    if "return" in code1:
                                        if len(param_lst2) <= 1:
                                            script += (
                                                line_script.replace(
                                                    "# TO_FILL_EXEC",
                                                    f"print(repr({func_name}({case_new1})))",
                                                )
                                                + "\n"
                                            )
                                        else:
                                            ...
                                            script += (
                                                ...
                                                + line_script.replace(
                                                    "# TO_FILL_EXEC",
                                                    f"print(repr({func_name}({case_new2})))",
                                                )
                                                + "\n"
                                            )
                                    else:
                                    ...
                                    script += line_script
                        with open(
                            f"./cleaned_data/test_case_gen_scripts/tmp_scripts/tmp.py",
                            "w",
                        ) as fw:
                            fw.write(script)
                        # print(script)
                        ...
                        try:
                            result = run_cmd(
                                f"python ./cleaned_data/test_case_gen_scripts/tmp_scripts/tmp.py",
                                timeout=timeout,
                            )
                        # signal.alarm(0)
            ...
            if keep_all:
                with jsonlines.open(
                    f"./cleaned_data/{model}/test_cases/test_cases_{dst_lang}_keep_all.jsonl",
                    "w",
                ) as fw:
                    fw.write({"id": id, "test_case": item_case_lst})
            else:
                with jsonlines.open(
                    f"./cleaned_data/{model}/test_cases/test_cases_{dst_lang}.jsonl",
                    "w",
                ) as fw:
...
def locate_function_name_java(code):
    pattern = re.compile(
        r"(public|private|protected)?\s?(static)?\s?(\w+|\w+\[\])\s(\w+)\s?\(\s?(\w+.*\w*)?\s?\)"
    )
...
def has_array(return_type):
    ...
    if (
        len(pattern.findall(return_type)) == 0
        and len(pattern1.findall(return_type)) == 0
    ):
        return False
    else:
        return True
...
def refine_code_ja(code):
    # 非贪婪方式，包括换行符
    pattern = re.compile(
        r"System\s?\.\s?out\s?\.\s?print[ln]*\s?\(\s?.*?\s?\)\s?;\n", flags=re.S
    )
    code = pattern.sub(";\n", code)
    return code
...
def post_process_ja(model, keep_all, dst_lang, timeout=5, start=1):
    ...
    count = 0
    with open(
        f"./cleaned_data/{model}/valid_inputs/valid_inputs_java.jsonl", encoding="utf-8"
    ) as fr:
        for line in tqdm(fr.readlines()):
            count += 1
            if count < start:
            ...
            for item_i in item:
                ...
                for case in case_lst:
                    ...
                    if return_type == "void":
                        ...
                        params = ",".join(var_lst)
                        single_return_type = (
                            return_type.replace("[", "").replace("]", "").strip()
                        )
                        script_case = (
                    ...
                    script = ""
                    with open(
                        "./cleaned_data/test_case_gen_scripts/gen_case.java",
                        encoding="utf-8",
                    ) as fr:
                        for line_script in fr.readlines():
                            if line_script.strip() == "// TO_FILL_FUNC":
                                ...
                                script += (
                                    line_script.replace("// TO_FILL_EXEC", script_case)
                                    + "\n"
                                )
                            else:
                                script += line_script
                    with open(
                        f"./cleaned_data/test_case_gen_scripts/tmp_scripts/tmp.java",
                        "w",
                    ) as fw:
                        fw.write(script)
                    # print(script)
                    ...
                    try:
                        result = run_cmd(
                            f"java ./cleaned_data/test_case_gen_scripts/tmp_scripts/tmp.java",
                            timeout=timeout,
                        )
                        print(
                            f"DEBUG: Java execution result - stdout: '{result[0]}', stderr: '{result[1]}'"
                        )
                    # signal.alarm(0)
                    except Exception as ret:
                        print("IGNORE THIS CASE - EXCEPTION:", ret)
                    ...
                    if return_type == "void":
                        ...
                        if "char" in return_type:
                            result = ",".join(
                                [f"'{i}'" for i in result.strip().split()]
                            )
...
def post_process_cp(model, keep_all, dst_lang, timeout=5, start=1):
    ...
    with open(
        f"./cleaned_data/{model}/valid_inputs/valid_inputs_cpp.jsonl", encoding="utf-8"
    ) as fr:
        for line in tqdm(fr.readlines()):
            count += 1
            if count < start:
            ...
            for item_i in item:
                ...
                for case in case_lst:
                    ...
                    script = ""
                    with open(
                        "./cleaned_data/test_case_gen_scripts/gen_case.cpp",
                        encoding="utf-8",
                    ) as fr:
                        for line_script in fr.readlines():
                            if line_script.strip() == "// TO_FILL_FUNC":
                                ...
                                script += line_script
                    with open(
                        f"./cleaned_data/test_case_gen_scripts/tmp_scripts/tmp.cpp", "w"
                    ) as fw:
                        fw.write(script)
                    # print(script)
                    signal.signal(signal.SIGALRM, TimeoutHandler)
                    ...
                    try:
                        result = run_cmd(
                            f"g++ ./cleaned_data/test_case_gen_scripts/tmp_scripts/tmp.cpp -o "
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--src_lang", type=str, help="source language", default="python"
    )
    parser.add_argument("--dst_lang", type=str, help="target language", default="cpp")
    parser.add_argument(
        "--keep_all", type=bool, help="keep all correct/incorrect cases", default=False
    )
    args = parser.parse_args()
    process_valid_inputs(
        model=args.model,
