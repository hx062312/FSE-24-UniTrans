...
# python deepseek.py --src_lang ${source_lang} --dst_lang ${target_lang} --obj 4 --k {sample_k} --test_case_num {test_case_num}
# python deepseek.py --src_lang python --dst_lang java --obj 4
# python deepseek.py --src_lang cpp --dst_lang python --obj 4
import re
import os
import openai
...
from cleaned_data.templates.examples_inp import (
    example_cpp,
    valid_inputs_cpp,
    example_java,
    valid_inputs_java,
...
from cleaned_data.templates.examples_trans import (
    ...
    example_code_cpp,
    example_code_python,
    example_test_cases_java,
    example_test_cases_cpp,
    example_test_cases_python,
)
from cleaned_data.templates.example_refine import (
    python_refine_example2_1,
    python_refine_example2_2,
    cpp_refine_example2_1,
    cpp_refine_example2_2,
    java_refine_example2_1,
    java_refine_example2_2,
)
...
def collect_one(prompt, api_key):
    ...
    response = client.chat.completions.create(
        model="deepseek-chat", messages=prompt, stream=False
    )
    ...
    # 处理格式转换：将 [Input_X] 转换为 Input_X:
    content = re.sub(r"\[(Input_\d+)\]", r"\1:", content)
...
def prompt_trans_w_case(src_lang, dst_lang, item, test_cases, test_case_num):
    ...
    example_src_code = eval(f"example_code_{src_lang}")
    example_test_cases = "\n".join(
        eval(f"example_test_cases_{dst_lang}")[:test_case_num]
    )
...
def deepseek(
    ...
    if obj == Obj.TRANS_W_CASES:
        test_case_lst = {}
        with open(
            f"./cleaned_data/deepseek/test_cases/test_cases_{dst_lang}.jsonl",
            encoding="utf-8",
        ) as fr:
            for line in fr.readlines():
                line = json.loads(line)
                test_case_lst.update(
                    {line["id"]: list(line["test_case"])[:test_case_num]}
                )
    if obj == Obj.REFINE:
        test_case_lst = {}
        with open(
    ...
    prefix = [
        {
            "role": "system",
            "content": "You are a professional developer proficient in java, python, and cpp.\n",
        }
    ]
    count = 0
    for id, item in tqdm(zip(sample_ids, data), total=len(sample_ids)):
        ...
        if obj == Obj.TRANS:
            ...
            if test_cases.strip() == "":
                ...
                target = [
                    prompt_trans_w_case(
                        src_lang, dst_lang, item, test_cases, test_case_num
                    )
                ]
        elif obj == Obj.REFINE:
            flag = True
            ...
            if flag is True:
                ...
                if feedback0["err_type"] == "REDO":
                    ...
                    target = [
                        prompt_refine(dst_lang, org_sols[id][0].strip(), feedback0)
                    ]
        else:
        ...
        with jsonlines.open(out_path, "w") as fw:
            if obj == Obj.GEN_VAL_INP:
                result = collect_one(prompt, api_key)
                print(f"API返回结果: {result}")  # 添加这行来查看API返回内容
                fw.write({"id": id, src_lang: result})
            else:
                result = collect_one(prompt, api_key)
                print(f"API返回结果: {result}")  # 添加这行来查看API返回内容
                fw.write({"id": id, dst_lang: result})
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--apikey", type=str, help="enter your api key", default=API_KEY
    )
    parser.add_argument("--src_lang", type=str, help="source language", default="java")
    parser.add_argument(
        "--dst_lang", type=str, help="target language", default="python"
    )
    ...
    parser.add_argument(
        "--test_case_num", type=int, help="num of test cases", default=10
    )
    parser.add_argument(
        "--obj",
        type=int,
        help="select an objective - 0: gen_val_inp, 2: trans, 3: trans_w_cases, 4: refine",
        default=0,
    )
    ...
    test_case_num = (
        args.test_case_num
    )  # todo only be activated when obj == TRANS_W_CASES

    test_file = os.path.join(Paths.TEST_DIR, "testable_samples.jsonl")
    feedback_file = os.path.join(
        Paths.FEEDBACK_DIR,
        f"testable_{src_lang}_{dst_lang}_w_{test_case_num}cases_{_round}round.jsonl",
    )
    org_sol_file = os.path.join(
        Paths.ORG_SOL_DIR,
        f"testable_{src_lang}_{dst_lang}_w_{test_case_num}cases_{_round}round.jsonl",
    )
    test_cases_file = os.path.join(Paths.TEST_CASES_DIR, f"test_cases_{dst_lang}.jsonl")
    valid_inputs_file = os.path.join(
        Paths.VALID_INPUTS_DIR, f"valid_inputs_{src_lang}.jsonl"
    )
    repair_file = os.path.join(
        Paths.REPAIR_DIR,
        f"repair_{src_lang}_{dst_lang}_w_{test_case_num}cases_{_round}round.jsonl",
    )
    if obj == Obj.GEN_VAL_INP:
