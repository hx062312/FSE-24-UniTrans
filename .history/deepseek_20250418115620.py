...
def prompt_case(src_lang, item):
    if src_lang == "cpp":
        return {"role": "user", "content": f"{example_cpp}\n{valid_inputs_cpp}\n{item}\nplease generate ten groups of differentiated valid inputs for the above focal method of {src_lang} language, in the format of [Input_1]\n[Input_2]\n...[Input_10]. Finally, use END_OF_CASE to finish your answer."}
    elif src_lang == "java":
...
def deepseek(data_path, src_lang, dst_lang, obj, sample_num, api_key, out_path, test_case_num, feedback_file, org_sol_file, start):
    ...
    if obj == Obj.TRANS_W_CASES:
        test_case_lst = {}
        with open(f"./cleaned_data/deepseek/test_cases/test_cases_{src_lang}.jsonl", encoding="utf-8") as fr:
            for line in fr.readlines():
                line = json.loads(line)
...
if __name__ == "__main__":
    ...
    test_cases_file = os.path.join(Paths.TEST_CASES_DIR, f"test_cases_{src_lang}.jsonl")
