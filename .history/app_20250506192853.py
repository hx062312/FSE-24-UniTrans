...
def process():
    ...
    try:
        if step == 'source_code_preservation':
            ...
            return jsonify({'success': True, 'translated_code': result['translated_code'], 'test_results': result['test_results']})
        else:
...
def handle_translation_augmentation(data):
    ...
    cmd3 = f"python fetch_feedbacks.py --model {model_name} --src_lang {source_lang} --dst_lang {target_lang} --test_case_num {test_case_num} --round {round_num}"
    subprocess.run(cmd1, shell=True, check=True, capture_output=True, text=True)
    subprocess.run(cmd2, shell=True, check=True, capture_output=True, text=True)
    subprocess.run(cmd3, shell=True, check=True, capture_output=True, text=True)
    with open(f"cleaned_data/{model_name}/post_processed/testable_{source_lang}_{target_lang}_w_10cases_2round.jsonl", "r") as f:
        data = json.loads(f.readline())
        translated_code = data[target_lang][0] if len(data[target_lang]) > 0 else "翻译失败"
    with open(f"cleaned_data/{model_name}/feedbacks/raw/testable_{source_lang}_{target_lang}_w_10cases_2round_raw.jsonl", "r") as f:
        data = json.loads(f.readline())
        test_results = data["feedbacks"]
    return {'success': True, 'translated_code': translated_code, 'test_results': test_results}
...
def handle_translation_repair(data):
    ...
    with open(post_processed_path, "r") as f:
        ...
        translated_code = data_post[target_lang][0] if len(data_post[target_lang]) > 0 else "翻译修复失败"
    ...
    return {'success': True, 'translated_code': translated_code, 'fetch_feedbacks_results': fetch_feedbacks_results,'test_results': test_results}
